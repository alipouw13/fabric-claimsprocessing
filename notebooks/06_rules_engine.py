...
# MAGIC %md
# MAGIC # Claims Rules Engine - Microsoft Fabric Version
# MAGIC 
# MAGIC **Dynamic rule-based claims processing system:**
# MAGIC - **Pre-defined checks** for routine claims processing without human intervention
# MAGIC - **Flag exceptions** when reported data doesn't match auto-detected information
# MAGIC - **Input**: silver_claim_policy_accident table
# MAGIC - **Rules Storage**: claims_rules table  
# MAGIC - **Output**: gold_insights table with applied rules

# COMMAND ----------

# MAGIC %md
# MAGIC ## Rules Engine Overview
# MAGIC 
# MAGIC **Dynamic Rules System:**
# MAGIC - ✅ **Add/edit rules** dynamically to meet business requirements
# MAGIC - ✅ **Persist rules** in claims_rules table for reusability
# MAGIC - ✅ **Apply rules** generically on new data
# MAGIC 
# MAGIC **Rule Definition Components:**
# MAGIC - 🔑 **Unique Rule ID/Name**
# MAGIC - 📋 **Business Logic** (written as executable SQL code)
# MAGIC - ⚠️ **Severity Level** (HIGH, MEDIUM, LOW)
# MAGIC - 🔘 **Active Status** (True/False)
# MAGIC 
# MAGIC **Common Business Rules:**
# MAGIC - 📅 Claim date within policy coverage period
# MAGIC - 🎯 Reported severity matches ML predicted severity
# MAGIC - 📍 Accident location matches telematics vs reported location
# MAGIC - 🚗 Speed compliance with regional limits
...
try:
    ...
    source_tables = ["silver_claim_policy_accident", "silver_claim_policy_location", "silver_claim_policy"]
...
try:
    ...
    # Optimize the table
    spark.sql("OPTIMIZE gold_insights")
    
    # Verify the save
    saved_count = spark.table("gold_insights").count()
    print(f"✅ Successfully saved {saved_count:,} records to gold_insights")
    
    print(f"\n📋 Gold table schema:")
    spark.table("gold_insights").printSchema()
...
try:
    ...
    insights_df = spark.table("gold_insights")
...
try:
    ...
    total_claims = spark.table("gold_insights").count()
